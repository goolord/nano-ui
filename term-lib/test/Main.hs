...
main = do
  ...
  run "id-zero-alloc" runIdZeroAllocTest
  run "id-keyed-list" runIdKeyedListTest
...
runIdStabilityTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 100 100}
...
runIdZeroAllocTest ctx failed = do
  ...
  when enabled $ do
    let
      inp = emptyInput {inputWindowSize = Size 1 1}
...
runIdKeyedListTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 200 200}
...
runFitSizingTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 400 100}
...
runLayoutTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 400 300}
  (_, _, draw, _) <-
    runFrame
      ctx
      inp
...
runOverlayTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 200 80}
...
runInteractionTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 200 100}
...
runTextInputCursorTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 320 120}
...
runModalOverlayTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 320 200}
    ui =
      column defaultLayout $ do
...
runModalNoPhantomScrollTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 400 300}
    ui =
      modal True "About" $ do
  ...
  let
    ctx' =
      withClipboard
        ...
        (\s -> writeIORef clipRef (Just s) >> pure True)
  let
    inp0 = emptyInput {inputWindowSize = Size 320 160}
    ui = column defaultLayout (textInput "Name" "hello")
  _ <- runFrame ctx' inp0 ui
  _ <- runFrame ctx' inp0 ui
  let
...
runSliderCursorTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 300 80}
...
runScrollThumbCursorTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 200 120}
    ui =
      scrollArea
...
runGrowScrollGutterTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 240 140}
    ui = do
      (sid, child) <-
        scrollArea
...
runWindowScrollGutterTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 640 360}
...
runTextInputSpanTest ctx failed = do
  ...
  spans <- collectTextSpans ctx
  let
    hasHello = any (\(_, txt, _, _, _) -> "hello" `T.isInfixOf` txt) spans
...
runButtonPressReleaseHoverTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 200 100, inputDeltaTime = 0.016}
    ui = column defaultLayout (button "Hover")
  _ <- runFrame ctx inp0 ui
  let
    click = V2 10 10
...
runOverlayPanelLiveTest _ failed = do
  let
    inp = emptyInput {inputWindowSize = Size 320 240, inputMousePos = V2 (-10) (-10)}
    checkStatic ui = do
      ctx <- newContext
      _ <- runFrame ctx inp ui
      _ <- runFrame ctx inp ui
...
runAnimationIdleTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 0.05}
...
runAnimationSettleTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 0.1}
    wid = WidgetId 99
...
runAnimationEaseTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 0.5}
    wid = WidgetId 100
...
runAnimationHoldTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 200 100, inputDeltaTime = 0.1}
    ui = do
      t <- animateTo 1 0.2
  ...
  when (not (any (\t -> t == "1.0" || "1.0" `T.isPrefixOf` t) shown)) $
    bump failed
...
runAnimationDamageTest _ failed = do
  ctx <- newContext
  let
    idleInp = emptyInput {inputWindowSize = Size 200 100, inputDeltaTime = 0}
    idle = label_ "anim"
...
runAnimationDelayTest _ failed = do
  ctx <- newContext
  let
    inp0 = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 0.1}
    wid = WidgetId 202
...
runAnimationStaggerTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 200 100, inputDeltaTime = 0.02}
    ui = do
...
runAnimationSpringRetargetTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 0.02}
    wid = WidgetId 402
...
runAnimationSpringDtTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 100 100, inputDeltaTime = 2}
    wid = WidgetId 403
...
runAnimationSpringHoldTest _ failed = do
  ctx <- newContext
  let
    inp = emptyInput {inputWindowSize = Size 200 100, inputDeltaTime = 0.05}
    ui = do
...
runAnimationSharedCtxTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 80 80, inputDeltaTime = 0.1}
    wid = WidgetId 777
...
runSliderFillWidthTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 400 120}
...
runNestedScrollTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 200 200}
    ui = do
      (outer, inner) <-
        scrollArea
...
runNestedScrollFocusTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 240 220}
    ui = do
      (outer, (inner, btn)) <-
        scrollArea
          (defaultLayout {layoutWidth = Grow 1, layoutHeight = Fixed 90})
...
runSelectTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 320 80}
...
runTextWrapAssignedTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp = emptyInput {inputWindowSize = Size 20 12}
    long = T.replicate 24 (T.pack "x")
    ui =
      column
        ( defaultLayout
...
runFlexWrapTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp = emptyInput {inputWindowSize = Size 30 10}
    ui =
      row
        ( defaultLayout
            { layoutWrap = True
...
runGrowFitsWindowTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 10 10}
    ui =
      row
        ( defaultLayout
            { layoutWidth = Grow 1
...
runAspectLayoutTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 320 240}
    ui =
...
runGrowWrapPushesSiblingTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp = emptyInput {inputWindowSize = Size 6 20}
    ui =
      column
        ( defaultLayout
            { layoutWidth = Grow 1
...
runTerminalSliderTrackTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp0 = emptyInput {inputWindowSize = Size 60 10}
    ui = column (fillW defaultLayout) (slider "Vol" 0 100 0)
  _ <- runFrame ctx inp0 ui
  ((resp, _), _, _, _) <- runFrame ctx inp0 ui
  let
...
runTerminalModalScrollTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp0 = emptyInput {inputWindowSize = Size 80 16}
    line1 = T.pack "line 1"
    ui = do
      (dlg, _) <-
        modal True "About"
  ...
  let
    titleYs0 = spanYs (T.pack "About") spans0
  ...
  case line1Ys0 of
    ...
    b0 : _ -> do
      ...
      let
        titleYs1 = spanYs (T.pack "About") spans1
...
runTerminalModalOpenRedrawTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp0 = emptyInput {inputWindowSize = Size 80 24, inputMousePos = V2 (-10) (-10)}
    ui = do
      (readOpen, setOpen) <- useFlag False
      resp <- button "Open"
      onClick resp (setOpen True)
...
runTerminalButtonBracketTest _ failed = do
  ...
  let
    pageBrackets = bracketSpans baseP
...
runTerminalWideClearBracketTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 2 1}
    clip = Rect 0 0 2 1
...
runTerminalWideCursorCupTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 4 1}
    clip = Rect 0 0 4 1
    fg = colorRGBA 220 220 220 255
    bg = colorRGBA 20 20 24 255
  (_, _, draw, _) <- runFrame ctx inp (pure ())
...
runTerminalWidePairTest _ failed = do
  ...
  let
    skip = oneColFaOrigins (base ++ overlay)
    rows = cellRows cells
    broken =
      [ (r, c)
...
runHostSlotTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 80 80}
...
runReduceClickTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 240 120}
    view m = do
      resp <- button "Go"
  ...
  (_, modelR, msgs, _, dirty) <-
    runFrameReduce updateCounter ctx release modelP view
...
runOverlayClickThroughTest _ failed = do
  ctx <- newContext
  let
    inp0 = emptyInput {inputWindowSize = Size 300 220}
    windowUi = do
    ...
    clickNone clicked ui pos = do
      let
        (press, release) = clickAt inp0 pos
      _ <- runFrame ctx press ui
    ...
    runCovered ui = do
      ...
      case mInside0 of
        ...
        Just inside0 -> do
          ...
          let
            (ipress, irelease) = clickAt inp0 ip
...
runWindowScrollWheelTest ctx failed = do
  let
    inp0 = emptyInput {inputWindowSize = Size 320 220}
    line1 = T.pack "line 1"
    ui = do
      (win, _) <-
        window True "Scroll"
  ...
  spans0 <- collectOverlayTextSpans ctx inp0
  let
    titleYs0 = spanYs (T.pack "Scroll") spans0
    line1Ys0 = spanLabelYs line1 spans0
  when (null titleYs0) $ bump failed
  case line1Ys0 of
    [] -> bump failed
    b0 : _ -> do
      ...
      spans1 <- collectOverlayTextSpans ctx wheel
      let
        titleYs1 = spanYs (T.pack "Scroll") spans1
        line1Ys1 = spanLabelYs line1 spans1
      case (titleYs0, titleYs1) of
        (y0 : _, y1 : _) -> when (y1 /= y0) $ bump failed
        _ -> bump failed
...
runSeparatorSpanTest ctx failed = do
  let
    inp = emptyInput {inputWindowSize = Size 200 120}
    ui =
...
runTerminalSeparatorSpanTest _ failed = do
  ctx <- newAdaptiveTerminalContext
  let
    inp = emptyInput {inputWindowSize = Size 40 8}
    ui =
      column (fillW defaultLayout) $ do
        label_ "A"
        resp <- separator
...
runTabsLazinessTest ctx failed = do
  ...
  let
    inp = emptyInput {inputWindowSize = Size 200 100}
    ui = do
      _ <-
        tabs
          TabB
...
runTabsDamageTest _ failed = do
  ctx <- newContext
  let
    inp0 = emptyInput {inputWindowSize = Size 300 100, inputMousePos = V2 (-10) (-10)}
    ui curTab =
      tabs
        curTab
...
runTabsContentDamageTest _ failed = do
  ctx <- newContext
  let
    inp0 = emptyInput {inputWindowSize = Size 320 200, inputMousePos = V2 (-10) (-10)}
    ui = do
