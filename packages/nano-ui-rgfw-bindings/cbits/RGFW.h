...
void RGFW_waitForEvent(i32 waitMS) {
	...
	if (_RGFW->eventWait_forceStop[0] == 0 || _RGFW->eventWait_forceStop[1] == 0) {
		if (pipe(_RGFW->eventWait_forceStop) != -1) {
			/* nano-ui: set O_NONBLOCK, as upstream intended, so a stop never
			   blocks on a full pipe nor a drain on an empty one. */
    ...
    fds[0].events = POLLIN;
    fds[0].revents = 0;
    /* nano-ui: if pipe() failed, poll nothing rather than stdin. */
	...
	if (RGFW_usingWayland()) {
		...
		while (XPending(_RGFW->display) == 0) {
			/* nano-ui: poll the stop pipe too, so RGFW_stopCheckEvents
			   from another thread ends the wait. */
	...
	/* nano-ui: drain the whole pipe whenever it has data. Stops sent while
	   the loop was not waiting pile up, and a leftover byte with the flag
	   clear would end every later wait at once (a busy loop). */
