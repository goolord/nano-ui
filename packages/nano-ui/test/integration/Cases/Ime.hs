-- | Input-method composition: it shows at the focused field's caret without
-- changing the value, its keys edit nothing, its commit types, and the frame
-- says where the candidate window goes.
...
-- | @i@ with an input method composing @txt@, its caret @cursor@ characters in.
composing :: T.Text -> Int -> Input -> Input
composing txt cursor = applyComposition txt cursor 0

-- | @i@ with the composition ended and @txt@ committed.
...
-- | Every text of @yes@ is a span of the frame, and none contains one of @no@.
...
-- | @widget@ holding @v@ in a column, Tabbed onto, its caret @back@
...
-- | Held until it changes or ends; unset or out-of-range positions clamp.
...
-- | Shown at the caret in place of any selection, in plain, search and
-- (masked) password fields; the value and change flag stay put.
...
-- | A commit inserts at the caret and ends the composition, before a new one
-- it comes with; an empty field shows the composition over its placeholder.
...
-- the focus, and its Escape quits nothing. An empty composition is none.
...
-- | A text area shows it on the caret's row, or over a selection of several
-- rows in place of the first row's part; Enter does nothing, and the commit
...
-- | Where the focused field takes text ('textInputArea'), how paint draws the
-- composition, and where a press over it lands, in a field and on a text
-- area's row; no area once nothing has the focus.
...
runImeAreaPaintTest ctx failed = do
  ...
  let cell = 16
      ...
      -- On the row showing @line@, a composition @len@ cells long from cell
      -- @at@, with the input method's caret @c@ cells into it.
  ...
  assertJust failed (spanRectOf "abcd" spans) $ \(Rect sx sy _ sh) -> do
    ...
    -- Between the shown "c" and "d" is after the value's "c".
...
-- | Past a narrow field's edge it scrolls the field to keep the input
-- method's caret in view, and back as that caret returns to the start.
...
-- runs right to left, its first caret at its right edge.
...
-- focus leaves it in no field, whose keys work again, until it changes there.
...
runImeOwnerTest ctx failed = do
  ...
  -- With no focus it goes nowhere, and Tab still moves.
  warmup ctx inp ui
  ...
  -- Left behind unchanged, it stays out of the field Tab reached.
  shown ctx failed [] ["x"]
...
-- | Starting, changing or ending it repaints its field on an idle frame.
...
-- | A view run again after a hook write shows it and commits it once.
...
-- | No shortcut fires on its keys: composing, the frame drops presses,
-- releases and held keys; in the frame it commits, the keys passed on reach
-- the field but fire nothing, and the commit goes in though Ctrl is held.
