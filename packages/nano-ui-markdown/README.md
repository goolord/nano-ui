...
Append a chat reply's tokens as they arrive, once a frame, with the tokens
that arrived in that frame joined:
...
appending the tokens of a frame one at a time costs more for nothing.
...
Two documents are equal when their texts are, so one kept in `useState`
...
`cabal run nano-ui-markdown-example` streams a reply into a chat view. It
...
and heading fonts, the link and code colours, whether code blocks have a copy
button, and `mdImage`, which resolves an image's source to a registered image
and a size. An image alone in its paragraph, or alone in a link, is drawn when
`mdImage` resolves it; a click goes to the link, else to its source, and its
title (else the link's) shows as a tooltip. Any other image shows its alt text
as a link. Long code lines wrap, keeping their indentation, and headings scale
the backend's default font size. Task items draw nano-ui's checkbox.

```haskell
markdownConfigured
  defaultMarkdownConfig {mdText = fontSize 15, mdImage = \src -> lookup src loadedImages}
  doc
```
