...
* Text op cache for static labels (leaf `NodeText`): placed spans are keyed on widget, content, font selection, colors, alignment, and wrap width, and are reused while unchanged — measure/wrap/truncate run only on refresh frames, so HUD-style label walls cost ~0 between refreshes. A move translates the cached spans; a resize, theme/color change, or content change rebuilds. Entries for labels that stop painting are pruned after a two-frame grace (`pruneDrawOpCache`).
