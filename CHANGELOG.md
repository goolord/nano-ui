...
* Wait pacing: when the previous frame actually presented, the loop waits 0 ms and lets vsync throttle (frames lock to the display). When frames skip (empty damage, e.g. an animation scrolled out of view), it paces at `animateTimeout` so the loop does not busy-spin.
