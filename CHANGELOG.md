...
* SDL3 pkg-config is only required when building `nano-ui-sdl` with `-fsdl` (TUI and headless tests do not need SDL3).
* TUI and SDL live in standalone packages (`nano-ui-term`, `nano-ui-sdl`). Core uses `HostProfile` (`PixelHost` / `CellHost`) instead of inferring a terminal from font metrics. `newTerminalContext` / `terminalTheme*` come from `NanoUI.Backend.Term`. `newSdlContext` / `sdlTheme` come from `NanoUI.Backend.Sdl`. `isTerminalFont` is gone. The test suite lives on `nano-ui-term` so cabal does not see a package cycle.
