...
description: Windows MSYS2/UCRT64 SDL3 builds for nano-ui. Trigger: Cabal-7107, sdl3 pkg-config missing, PKG_CONFIG_PATH.
---
...
1. Confirm the failure is missing SDL3 **dev files**, not a Haskell version conflict. A DLL next to the exe is runtime only. On every platform Cabal finds SDL3 through pkg-config (`sdl3.pc`, `sdl3-ttf.pc`).
...
3. Do not turn the `sdl` flag off (`-f-sdl`) to "fix" the error. The user asked for the SDL demo.
